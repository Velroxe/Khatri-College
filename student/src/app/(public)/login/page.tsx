...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = isOtpLogin
        ? `${host}/api/auth/student/login-otp`
        : `${host}/api/auth/student/login-password`;
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const res = await fetch(`${host}/api/auth/student/send-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div
      className={`relative min-h-screen flex items-center justify-center p-6 transition-none ${resolvedTheme === "dark"
        ? "bg-gray-950 text-gray-100"
        : "bg-gray-50 text-gray-900"
        }`}
    >
      <Card
        className={`w-full max-w-sm shadow-lg border ${resolvedTheme === "dark"
